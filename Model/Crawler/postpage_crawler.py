...
def post_crawler(post_url, zipfile_name):
    ...
    for single_push in pushes:
        ...
        push_userid = single_push.find(
            "span", class_="push-userid").text.strip()
        push_content = single_push.find(
            "span", class_="push-content").text.split(":")[1].strip()
        ...
        comment = {"status": push_tag, "comment_id": push_userid,
                   "content": push_content, "ip": push_ip, "comment_time": push_time}
    ...
    for pic in photo_hrefs:
        if 'imgur' in pic["href"] and 'https' in pic["href"]:
            ...
            pic.extract()
        if 'pbs.twimg' in pic["href"] and 'https' in pic["href"]:
            img_l.append(pic["href"])
            pic.extract()
    if img_l.count == 0:
        print(f"img抓取量為0:{post_url}")
    ...
    # (回應使用者的文字資訊, 回傳圖片檔名list)
    return post_time, author, comments, img_name_list, img_url_list
